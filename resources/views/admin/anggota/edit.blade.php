    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">Perbarui informasi anggota LSM GNRI</p>
            </div>
    ...
    <div class="py-8 bg-slate-50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm border border-slate-200 rounded-2xl">
                <form action="{{ route('admin.anggota.update', $anggota->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="email" name="email" value="{{ old('email', optional($anggota->user)->email) }}" required autocomplete="new-email"
                            ...
                            <!-- Password (Opsional) -->
                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Ganti Password <span class="text-slate-400 font-normal">(Kosongkan jika tidak diubah)</span></label>
                        ...
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 mt-4">
                            <div>
                                <label for="is_active" class="block text-xs font-bold text-slate-700 uppercase">Status Akun Anggota</label>
                                <p class="text-xs text-slate-500">Anggota non-aktif tidak akan bisa login ke sistem.</p>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_active" value="1" id="is_active" class="sr-only peer" 
                                    {{ old('is_active', $anggota->is_active ?? true) ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $anggota->tempat_lahir) }}" required
                            ...
                            <div>
                                ...
                                <select name="kabupaten_id" required
                                    ...
                                    @foreach($kabupatens as $kab)
                                        <option value="{{ $kab->id }}" {{ old('kabupaten_id', $anggota->kabupaten_id) == $kab->id ? 'selected' : '' }}>
                                            {{ $kab->nama_kabupaten }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="alamat" rows="3" required
                                          class="w-full text-sm rounded-xl border-slate-300 focus:border-emerald-500 focus:ring-emerald-500 @error('alamat') border-red-500 @enderror">{{ old('alamat', $anggota->alamat) }}</textarea>
                                @error('alamat')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
